...
class TestHipChatNotifier(TestCase):
    ...
    def setUp(self):
        ...
        self.redis_storage = MagicMock(RedisStorage)
        self.hipchat_client = MagicMock(HipChat)
        self.alert = MagicMock(Alert)

        self.config.get = self.mock_get

        self.hcn = HipChatNotifier(self.redis_storage, self.config)
        self.hcn._client = self.hipchat_client
    ...
    def test_should_not_notify_hipchat_if_no_rooms_have_been_added(self):
        ...
        self.assertEqual(self.hipchat_client.message_room.mock_calls, [])

    def test_should_not_notify_hipchat_if_warning_and_already_notified(self):
        self.redis_storage.is_locked_for_domain_and_key.return_value = True
        ...
        self.assertEqual(self.hipchat_client.mock_calls, [])
    ...
    def test_should_notify_hipchat_resolved_if_nominal_and_had_notified(self):
        ...
        self.redis_storage.\
            is_locked_for_domain_and_key.\
            assert_called_once_with('HipChat', self.alert_key)
        self.hipchat_client.message_room.assert_called_once_with(
            room_name,
            'Graphite-Pager',
            self.html_description,
            message_format='html',
        ...
        self.redis_storage.\
            remove_lock_for_domain_and_key.\
            assert_called_once_with('HipChat', self.alert_key)

    def test_should_notify_room_of_warning_if_had_not_notified_before(self):
        room_name = 'ROOM NAME'
        self.hcn.add_room(room_name)
        self.redis_storage.is_locked_for_domain_and_key.return_value = False
        ...
        self.redis_storage.set_lock_for_domain_and_key.assert_called_once_with(
